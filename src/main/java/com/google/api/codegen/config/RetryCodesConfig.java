...
public class RetryCodesConfig {
  ...
  private void populateRetryCodesDefinitionWithProtoFile(
    ...
    if (compareUnorderedLists(
        RETRY_CODES_FOR_HTTP_GET, retryCodesDefinition.get(RETRY_CODES_IDEMPOTENT_NAME))) {
    ...
    String noRetryName;
    if (compareUnorderedLists(
        RETRY_CODES_FOR_HTTP_NON_GET, retryCodesDefinition.get(RETRY_CODES_NON_IDEMPOTENT_NAME))) {
  ...
  private static boolean compareUnorderedLists(List<String> l1, List<String> l2) {
    if (l1.size() != l2.size()) {
      return false;
    }
    return Sets.newHashSet(l1).containsAll(l2);
  }
}
